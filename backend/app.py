...
def create_app():
    ...
    app.config['UPLOAD_FOLDER'] = os.path.join(os.path.dirname(__file__), '..', 'content', 'images', 'projects')
    ...
    with app.app_context():
        ...
        if contact_endpoint:
            ...
            app.view_functions['contact.submit_contact'] = limiter.limit("5 per hour")(contact_endpoint)
    ...
    def handle_server_error(e):
        ...
        app.logger.error(f"Internal Server Error: {request.path} - {str(e)}")
        return jsonify({
            'error': {
                'code': 'SERVER_ERROR',
