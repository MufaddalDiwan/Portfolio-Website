...
def submit_contact():
    ...
    try:
        ...
        logger.info(f"Contact form submission from {validated_data['name']} ({validated_data['email']}) - ID: {contact_message.id}")
        ...
        if not email_sent:
            ...
            logger.warning(f"Failed to send email notification for contact message ID: {contact_message.id}")
