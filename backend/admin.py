    ...
    def on_model_change(self, form, model, is_created):
        ...
        if model.slug:
            existing = Project.query.filter(
                Project.slug == model.slug, Project.id != model.id
            ).first()
            if existing:
                raise ValueError(
                    f"Slug '{model.slug}' already exists. Please choose a different title or slug."
                )
    ...
    def on_model_change(self, form, model, is_created):
        ...
        if hasattr(form, "bullets") and form.bullets.data:
            if isinstance(form.bullets.data, str):
                model.bullets = [
                    bullet.strip() for bullet in form.bullets.data.split("\n") if bullet.strip()
                ]
...
class ContactMessageAdmin(AuthenticatedModelView):
    ...
    column_descriptions = {"replied": "Check this box when you have responded to the message"}
...
def init_admin(app):
    ...
    admin.add_view(
        ExperienceAdmin(Experience, db.session, name="Experience", endpoint="admin_experience")
    )
    admin.add_view(
        ContactMessageAdmin(
            ContactMessage, db.session, name="Contact Messages", endpoint="admin_contact"
        )
    )
