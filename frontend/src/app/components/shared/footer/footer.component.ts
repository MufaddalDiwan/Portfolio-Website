...
export class FooterComponent implements OnInit {
  ...
  private loadSocialLinks(): void {
    this.apiService.getMeta().subscribe({
      next: (meta: SiteMeta) => {
        this.socialLinks = meta.socialLinks || [];
        this.loading = false;
      },
      error: (error) => {
        ...
        this.loading = false;
        // Fallback to empty array - footer will still show copyright
        this.socialLinks = [];
      },
    });
  }
  ...
  /**
   * Get the SVG icon for a social platform
   */
  ...
  /**
   * Get accessible label for social link
   */
  getSocialLabel(platform: string): string {
    ...
    return labels[platform.toLowerCase()] || `${platform} Profile`;
  }

  /**
   * Track by function for social links
   */
